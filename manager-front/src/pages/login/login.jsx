    ...
    <>
      <Col className="gutter-row box-main" xs={24} sm={24} md={24} lg={12} >
        <Form layout="vertical" className='box-form' onFinish={handleSignIn} >
          ...
          <Form.Item label="Senha" name="senha" rules={[{ required: true, message: "Senha é obrigatório" }]} >
          ...
          <Row justify='end'>
            <a className='box-retrieve-password' onClick={() => navigate('/forgot-password')} >Esqueceu a senha?</a>
          </Row>

          <Button block htmlType="submit" disabled={loading} size='large'>{loading ? <LoadingOutlined /> : 'ENTRAR'}</Button>
        </Form>
      </Col>
    </>
  )
